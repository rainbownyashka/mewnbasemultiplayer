    ...
    private void dequantize_sample(float[][] xr, int ch, int gr) {
        ...
        for (j = 0; j < this.nonzero[ch]; ++j) {
            ...
            if (gr_info.window_switching_flag != 0 && (gr_info.block_type == 2 && gr_info.mixed_block_flag == 0 || gr_info.block_type == 2 && gr_info.mixed_block_flag != 0 && j >= 36)) {
                ...
                int n = reste;
                fArray[n] = fArray[n] * two_to_negative_half_pow[idx += gr_info.subblock_gain[t_index] << 2];
            } else {
                ...
                float[] fArray = xr_1d[quotien];
                int n = reste;
                fArray[n] = fArray[n] * two_to_negative_half_pow[idx <<= gr_info.scalefac_scale];
            }
            ++index;
        }
    ...
    public void inv_mdct(float[] in, float[] out, int block_type) {
        ...
        if (block_type == 2) {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                int n8 = six_i + 6;
                out[n8] = out[n8] + (tmpf_0 *= 0.13052619f);
                int n9 = six_i + 7;
                out[n9] = out[n9] + tmpf_1;
                int n10 = six_i + 8;
                out[n10] = out[n10] + tmpf_2;
                int n11 = six_i + 9;
                out[n11] = out[n11] + tmpf_3;
                int n12 = six_i + 10;
                out[n12] = out[n12] + tmpf_4;
                int n13 = six_i + 11;
                out[n13] = out[n13] + tmpf_5;
                int n14 = six_i + 12;
                out[n14] = out[n14] + tmpf_6;
                int n15 = six_i + 13;
                out[n15] = out[n15] + tmpf_7;
                int n16 = six_i + 14;
                out[n16] = out[n16] + tmpf_8;
                int n17 = six_i + 15;
                out[n17] = out[n17] + tmpf_9;
                int n18 = six_i + 16;
                out[n18] = out[n18] + tmpf_10;
                int n19 = six_i + 17;
                out[n19] = out[n19] + tmpf_11;
                six_i += 6;
            }
        } else {
            in[17] = in[17] + in[16];
