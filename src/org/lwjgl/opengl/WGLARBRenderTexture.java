...
public class WGLARBRenderTexture {
    ...
    public static boolean wglBindTexImageARB(@NativeType(value="HPBUFFERARB") long pbuffer, int buffer) {
        long __functionAddress = GL.getCapabilitiesWGL().wglBindTexImageARB;
        if (Checks.CHECKS) {
            Checks.check(__functionAddress);
            Checks.check(pbuffer);
        }
        return JNI.callPI(pbuffer, buffer, __functionAddress) != 0;
    }

    @NativeType(value="BOOL")
    public static boolean wglReleaseTexImageARB(@NativeType(value="HPBUFFERARB") long pbuffer, int buffer) {
    ...
    public static int nwglSetPbufferAttribARB(long pbuffer, long attribList) {
        ...
        return JNI.callPPI(pbuffer, attribList, __functionAddress);
    }

    @NativeType(value="BOOL")
    public static boolean wglSetPbufferAttribARB(@NativeType(value="HPBUFFERARB") long pbuffer, @Nullable @NativeType(value="int const *") IntBuffer attribList) {
        if (Checks.CHECKS) {
            Checks.checkNTSafe(attribList);
        }
        return WGLARBRenderTexture.nwglSetPbufferAttribARB(pbuffer, MemoryUtil.memAddressSafe(attribList)) != 0;
    }

    @NativeType(value="BOOL")
    public static boolean wglSetPbufferAttribARB(@NativeType(value="HPBUFFERARB") long pbuffer, @Nullable @NativeType(value="int const *") int[] attribList) {
        long __functionAddress = GL.getCapabilitiesWGL().wglSetPbufferAttribARB;
        if (Checks.CHECKS) {
            Checks.check(__functionAddress);
            Checks.check(pbuffer);
        ...
        return JNI.callPPI(pbuffer, attribList, __functionAddress) != 0;
    }
}
