...
import com.cairn4.moonbase.tiles.behaviors.ItemRecycler;
...
extends BaseModule {
    ...
    public Recycler(World world, Chunk chunk, int x, int y) {
        ...
        this.itemRecycler = new ItemRecycler();
        this.itemRecycler.baseModule = this;
        this.itemRecycler.setFinishedIcon(this.buildFinishedIcon);
        this.behaviorList.add(this.itemRecycler);
        this.setupPhysics("smelter");
        this.addSmoke();
        this.itemCrafter = new ItemCrafter();
        this.itemCrafter.buildQueueSizeLimit = 1;
        this.itemCrafter.buildSpeedMultiplier = 2.0f;
    ...
    protected void setBuilderId() {
        this.builderId = "recycler";
    }

    @Override
    public void startBehaviors() {
        this.itemRecycler.start();
        this.itemCrafter.start();
        if ((this.itemRecycler.isDeconstructing() || this.itemCrafter.isBuilding()) && this.animGroup != null) {
    ...
    public void interact(Player player) {
        ...
        this.itemRecycler.player = player;
        this.itemRecycler.playerInventory = this.playerInventory;
        this.itemCrafter.player = player;
        this.itemCrafter.playerInventory = this.playerInventory;
        if (this.itemRecycler != null) {
            if (this.itemRecycler.itemsToCollect.size > 0) {
                boolean collected = this.itemRecycler.collectItem();
                if (!collected) {
            ...
            } else if (this.itemCrafter.itemsToCollect.size > 0) {
                boolean collected = this.itemCrafter.collectItem();
                if (!collected) {
                    String message = Localization.get("cantCollectNotification");
                    this.world.gameScreen.hud.hudNotifications.newMessage(message, Color.RED);
                }
            } else if (this.playerInventory.getEquippedItem() != null) {
                this.handleDroppedItem(this.playerInventory, this.playerInventory.getEquippedItem());
            }
        } else {
    ...
    public void handleDroppedItem(PlayerInventory playerInventory, ItemStack stack) {
        if (!this.itemRecycler.isDeconstructing() && !this.itemCrafter.isQueueFull()) {
            if (stack.getId().equals("refined-scrap")) {
                ...
                this.handleDropItemAnimation();
            } else if (this.itemRecycler.isItemDeconstructable(stack.getId())) {
                boolean goodToGO = true;
                ItemData id = stack.item.getData();
                if (id.craftingQuantity > 1) {
                    if (playerInventory.getAmountOfItem(stack.getId()) >= id.craftingQuantity) {
                        this.itemRecycler.playerInventory = playerInventory;
                        this.itemRecycler.player = this.player;
                        this.itemRecycler.addItemToQueue(stack.getId());
                        playerInventory.removeAmountOfItemId(id.craftingQuantity, stack.getId());
                        this.handleDropItemAnimation();
                    } else {
                        HudNotificationData msg = new HudNotificationData();
                        msg.icon = id.sprite;
                        msg.message = Localization.format("recycler_notEnoughItems", id.craftingQuantity);
                ...
                HudNotificationData msg = new HudNotificationData();
                msg.icon = "recycler-off";
                msg.message = Localization.get("recycler_cantRecycle");
                MessageManager.getInstance().dispatchMessage(3, msg);
            }
        }
    }
    ...
    public void update(float delta) {
        ...
        this.toggleFx((this.itemRecycler.isDeconstructing() || this.itemCrafter.isBuilding()) && this.hasPower);
    }
