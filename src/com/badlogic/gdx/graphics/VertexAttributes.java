...
Comparable<VertexAttributes> {
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < this.attributes.length; ++i) {
            result = result * 61L + (long)this.attributes[i].hashCode();
        }
    ...
    public int getBoneWeights() {
        if (this.boneWeightUnits < 0) {
            ...
            for (int i = 0; i < this.attributes.length; ++i) {
                VertexAttribute a = this.attributes[i];
                if (a.usage != 64) continue;
                this.boneWeightUnits = Math.max(this.boneWeightUnits, a.unit + 1);
            }
        }
    ...
    public int getTextureCoordinates() {
        if (this.textureCoordinates < 0) {
            ...
            for (int i = 0; i < this.attributes.length; ++i) {
                VertexAttribute a = this.attributes[i];
                if (a.usage != 16) continue;
                this.textureCoordinates = Math.max(this.textureCoordinates, a.unit + 1);
            }
        }
