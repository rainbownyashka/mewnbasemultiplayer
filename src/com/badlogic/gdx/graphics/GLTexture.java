...
implements Disposable {
    ...
    public float unsafeSetAnisotropicFilter(float level, boolean force) {
        float max = GLTexture.getMaxAnisotropicFilterLevel();
        if (max == 1.0f) {
            return 1.0f;
        }
        ...
        Gdx.gl20.glTexParameterf(3553, 34046, level);
    ...
    public float setAnisotropicFilter(float level) {
        ...
        if (MathUtils.isEqual(level = Math.min(level, max), this.anisotropicFilterLevel, 0.1f)) {
            return level;
        }
        this.bind();
        Gdx.gl20.glTexParameterf(3553, 34046, level);
    ...
    public static float getMaxAnisotropicFilterLevel() {
        ...
        if (Gdx.graphics.supportsExtension("GL_EXT_texture_filter_anisotropic")) {
            ...
            Gdx.gl20.glGetFloatv(34047, buffer);
