...
implements Disposable {
    ...
    public void beginCache(int cacheID) {
        ...
        if (cacheID == this.caches.size - 1) {
            ...
            ((Buffer)verticesBuffer).limit(oldCache.offset);
            this.beginCache();
            return;
        }
        this.currentCache = this.caches.get(cacheID);
        ((Buffer)verticesBuffer).position(this.currentCache.offset);
    }
    ...
    public int endCache() {
        ...
        int cacheCount = this.mesh.getVerticesBuffer(false).position() - cache.offset;
        if (cache.textures == null) {
            ...
            FloatBuffer vertices = this.mesh.getVerticesBuffer(true);
    ...
    public void draw(int cacheID, int offset, int length) {
        ...
        Cache cache = this.caches.get(cacheID);
        int verticesPerImage = this.mesh.getNumIndices() > 0 ? 4 : 6;
        offset = cache.offset / (verticesPerImage * 5) * 6 + offset * 6;
        length *= 6;
        Texture[] textures = cache.textures;
        int[] counts = cache.counts;
        int textureCount = cache.textureCount;
