...
extends SteeringBehavior<T> {
    ...
    protected SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> steering2) {
        ...
        Object targetPosition = this.target.getPosition();
        float squareDistance = steering2.linear.set(targetPosition).sub(this.owner.getPosition()).len2();
        ...
        steering2.linear.set(targetPosition).mulAdd(this.target.getLinearVelocity(), (float)predictionTime).sub(this.owner.getPosition()).nor().scl((float)this.getActualMaxLinearAcceleration());
