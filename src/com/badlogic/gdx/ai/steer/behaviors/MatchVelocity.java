...
extends SteeringBehavior<T> {
    ...
    protected SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> steering2) {
        steering2.linear.set(this.target.getLinearVelocity()).sub(this.owner.getLinearVelocity()).scl((float)(1.0f / this.timeToTarget)).limit(this.getActualLimiter().getMaxLinearAcceleration());
