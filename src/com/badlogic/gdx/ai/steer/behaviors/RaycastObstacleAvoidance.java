...
extends SteeringBehavior<T> {
    ...
    public RaycastObstacleAvoidance(Steerable<T> owner) {
        this(owner, (RayConfiguration<T>)null);
    }
    ...
    protected SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> steering2) {
        ...
        steering2.linear.set(this.minOutputCollision.point).mulAdd(this.minOutputCollision.normal, (float)(this.owner.getBoundingRadius() + this.distanceFromBoundary)).sub(this.owner.getPosition()).nor().scl((float)this.getActualLimiter().getMaxLinearAcceleration());
