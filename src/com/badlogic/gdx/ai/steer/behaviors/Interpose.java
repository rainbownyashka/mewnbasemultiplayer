...
extends Arrive<T> {
    ...
    protected SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> steering2) {
        ...
        steering2.linear.set(this.agentA.getPosition()).mulAdd(this.agentA.getLinearVelocity(), (float)timeToTargetPosition);
        this.internalTargetPosition.set(this.agentB.getPosition()).mulAdd(this.agentB.getLinearVelocity(), (float)timeToTargetPosition);
        this.internalTargetPosition.sub(steering2.linear).scl((float)this.interpositionRatio).add(steering2.linear);
        return this.arrive(steering2, this.internalTargetPosition);
    }
