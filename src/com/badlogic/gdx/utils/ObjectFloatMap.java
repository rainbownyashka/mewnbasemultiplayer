...
implements Iterable<Entry<K>> {
    ...
    public ObjectFloatMap(ObjectFloatMap<? extends K> map) {
        this((int)Math.floor((float)map.keyTable.length * map.loadFactor), map.loadFactor);
    ...
    public boolean containsValue(float value, float epsilon) {
        ...
        float[] valueTable = this.valueTable;
        for (int i = valueTable.length - 1; i >= 0; --i) {
            if (keyTable[i] == null || !(Math.abs(valueTable[i] - value) <= epsilon)) continue;
            return true;
        }
        return false;
    }
    ...
    public K findKey(float value, float epsilon) {
        ...
        for (int i = valueTable.length - 1; i >= 0; --i) {
            K key = keyTable[i];
            if (key == null || !(Math.abs(valueTable[i] - value) <= epsilon)) continue;
            return key;
        }
        return null;
    }
