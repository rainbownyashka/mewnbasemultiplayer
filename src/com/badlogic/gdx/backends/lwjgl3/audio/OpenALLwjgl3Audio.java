...
implements Lwjgl3Audio {
    ...
    public OpenALLwjgl3Audio(int simultaneousSources, int deviceBufferCount, int deviceBufferSize) {
        ...
        this.observerThread = new Thread(new Runnable(){
            ...
            public void run() {
                while (true) {
                    boolean isConnected;
                    boolean bl = isConnected = ALC10.alcGetInteger(OpenALLwjgl3Audio.this.device, 787) != 0;
                    if (!isConnected) {
                    ...
                    if (OpenALLwjgl3Audio.this.preferredOutputDevice != null) {
                        ...
                        String[] currentDevices = OpenALLwjgl3Audio.this.getAvailableOutputDevices();
                        ArrayList<String> currentDevicesList = new ArrayList<String>(Arrays.asList(currentDevices));
                        currentDevicesList.removeAll(Arrays.asList(this.lastAvailableDevices));
    ...
    public OpenALSound newSound(FileHandle file) {
        ...
        try {
            return soundClass.getConstructor(OpenALLwjgl3Audio.class, FileHandle.class).newInstance(this, file);
        }
        catch (Exception ex) {
    ...
    public OpenALMusic newMusic(FileHandle file) {
        ...
        try {
            return musicClass.getConstructor(OpenALLwjgl3Audio.class, FileHandle.class).newInstance(this, file);
        }
        catch (Exception ex) {
    ...
    private boolean switchOutputDevice(String deviceIdentifier, boolean setPreferred) {
        ...
        return SOFTReopenDevice.alcReopenDeviceSOFT(this.device, (CharSequence)deviceIdentifier, (IntBuffer)null);
    }
    ...
    int obtainSource(boolean isMusic) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            AL10.alSourcei(sourceId, 4147, 2);
            return sourceId;
        }
        return -1;
    }
