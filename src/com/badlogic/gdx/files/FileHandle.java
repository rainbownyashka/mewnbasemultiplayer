...
public class FileHandle {
    ...
    public byte[] readBytes() {
        ...
        try {
            byte[] byArray = StreamUtils.copyStreamToByteArray(input, this.estimateLength());
            return byArray;
        }
        catch (IOException ex) {
            throw new GdxRuntimeException("Error reading file: " + this, ex);
    ...
    public ByteBuffer map(FileChannel.MapMode mode) {
        ...
        try {
            ...
            MappedByteBuffer map = fileChannel.map(mode, 0L, f.length());
            map.order(ByteOrder.nativeOrder());
            mappedByteBuffer = map;
        }
        catch (Exception ex) {
            try {
                throw new GdxRuntimeException("Error memory mapping file: " + this + " (" + (Object)((Object)this.type) + ")", ex);
            }
            catch (Throwable throwable) {
