...
implements Disposable {
    ...
    public synchronized <T> boolean containsAsset(T asset) {
        ...
        for (RefCountedContainer assetRef : assetsByType.values()) {
            if (assetRef.object != asset && !asset.equals(assetRef.object)) continue;
            return true;
        }
        return false;
    }
    ...
    public synchronized <T> String getAssetFileName(T asset) {
        for (Class assetType : this.assets.keys()) {
            ...
            for (ObjectMap.Entry entry : assetsByType) {
    ...
    public <T> AssetLoader getLoader(Class<T> type, String fileName) {
        ...
        for (ObjectMap.Entry entry : loaders.entries()) {
            if (((String)entry.key).length() <= length || !fileName.endsWith((String)entry.key)) continue;
