...
extends TextField {
    ...
    protected int letterUnderCursor(float x) {
        if (this.linesBreak.size > 0) {
            ...
            for (i = start; i < end && !(glyphPositions[i] > x); ++i) {
            }
    ...
    protected void drawSelection(Drawable selection, Batch batch, BitmapFont font, float x, float y) {
        ...
        for (int i = this.firstLineShowing * 2; i + 1 < this.linesBreak.size && i < (this.firstLineShowing + this.linesShowing) * 2; i += 2) {
            ...
            if (!(minIndex < lineStart && minIndex < lineEnd && maxIndex < lineStart && maxIndex < lineEnd || minIndex > lineStart && minIndex > lineEnd && maxIndex > lineStart && maxIndex > lineEnd)) {
                ...
                if (lineFirst != null) {
                    if (start == lineStart) {
                        fontLineOffsetWidth = lineFirst.fixedWidth ? 0.0f : (float)(-lineFirst.xoffset) * fontData.scaleX - fontData.padLeft;
                    } else {
                        fontLineOffsetX = lineFirst.fixedWidth ? 0.0f : (float)(-lineFirst.xoffset) * fontData.scaleX - fontData.padLeft;
                    }
                }
    ...
    public float getCursorX() {
        ...
        if (this.cursor < this.glyphPositions.size && this.cursorLine * 2 < this.linesBreak.size) {
            ...
            BitmapFont.Glyph lineFirst = fontData.getGlyph(this.displayText.charAt(lineStart));
            if (lineFirst != null) {
                glyphOffset = lineFirst.fixedWidth ? 0.0f : (float)(-lineFirst.xoffset) * fontData.scaleX - fontData.padLeft;
            }
    ...
    public float getCursorY() {
        BitmapFont font = this.style.font;
        return (float)(-(this.cursorLine - this.firstLineShowing + 1)) * font.getLineHeight();
    }
