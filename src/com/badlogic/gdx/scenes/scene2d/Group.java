...
implements Cullable {
    ...
    public void addActorAfter(Actor actorAfter, Actor actor) {
        ...
        if ((index = this.children.indexOf(actorAfter, true)) == this.children.size || index == -1) {
            this.children.add(actor);
        } else {
    ...
    public <T extends Actor> T findActor(String name) {
        ...
        int n = children.size;
        for (i = 0; i < n; ++i) {
            if (!name.equals(((Actor)children.get(i)).getName())) continue;
            return (T)((Actor)children.get(i));
        }
        n = children.size;
        for (i = 0; i < n; ++i) {
            T actor;
    ...
    public Actor getChild(int index) {
        return (Actor)this.children.get(index);
    }
    ...
    public Vector2 localToDescendantCoordinates(Actor descendant, Vector2 localCoords) {
        ...
        if (parent == null) {
            throw new IllegalArgumentException("Actor is not a descendant: " + descendant);
        }
        if (parent != this) {
