...
implements Disposable {
    ...
    public void cancelTouchFocus(Actor listenerActor) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (focus.listenerActor != listenerActor || !touchFocuses.removeValue(focus, true)) continue;
            if (event == null) {
                event = Pools.obtain(InputEvent.class);
                event.setType(InputEvent.Type.touchUp);
                event.setStage(this);
    ...
    protected void actorRemoved(Actor actor) {
        int n = this.pointerOverActors.length;
        for (int pointer = 0; pointer < n; ++pointer) {
            if (actor != this.pointerOverActors[pointer]) continue;
            this.pointerOverActors[pointer] = null;
    ...
    public boolean setKeyboardFocus(@Null Actor actor) {
        ...
        boolean bl = success = !event.isCancelled();
        if (success) {
            ...
            if (actor != null) {
                ...
                actor.fire(event);
                boolean bl2 = success = !event.isCancelled();
                if (!success) {
    ...
    public Vector2 stageToScreenCoordinates(Vector2 stageCoords) {
        ...
        stageCoords.y = (float)Gdx.graphics.getHeight() - stageCoords.y;
        return stageCoords;
    }
